        ...
        try
        {
            ...
            await foreach (var data in _packetChannel.Reader.ReadAllAsync(ct))
            {
                ...
                if (packetsInCurrentBatch >= batchSize)
                {
                    ...
                    Console.WriteLine($"New batch with {totalBytes/(1024.0*1024.0):F2} MB written to Disk, queue currently at {_packetChannel.Reader.Count} packages, this write took {diskTimer.ElapsedMilliseconds} ms, the total filesice is now at {fileSizeBytes/(1024.0*1024.0*1024.0):F3} GB.");
                }
            }
        }
